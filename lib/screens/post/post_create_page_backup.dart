...
//     print(cityValue);
  ...
  Future<void> _storePost(BuildContext context) async {
    ...
    try {
      ...
      print('step 1');
      final beforeCapitalLetter = RegExp(r"(?=[A-Z])");
      var parts = countryValue.isEmpty ? '  '.split('') : countryValue.split(beforeCapitalLetter);
      parts.removeAt(0);
      String parseCountryVal = parts.join().trim();
      ...
      var request = http.MultipartRequest("POST",Uri.parse(api_endpoint + "api/v1/post"));
      request.headers['Authorization'] = "Bearer " + token!;

      images.forEach((element) {
        request.files.add(http.MultipartFile.fromBytes('images[]', File(element.image.path).readAsBytesSync(), filename: element.image.path));
      });
      ...
      request.fields['category_id'] = category_id == 0 ? 'null' : category_id.toString();
      request.fields['parent_category_id'] = id;
      request.fields['start_date'] = "${selectedDate.year}-${selectedDate.month}-${selectedDate.day}";
      request.fields['country'] = parseCountryVal;
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        children: [
          ...
          isSubmit ? 
          CircularProgressIndicator(color: primaryColorRGB(1),)
          :
          Container(
              width: double.infinity,
